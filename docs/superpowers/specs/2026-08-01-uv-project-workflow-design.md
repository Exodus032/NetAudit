...
- Update README setup commands to use `uv sync --project backend` and `uv run --directory backend -m netaudit.server`.
...
- Starting `uv run --directory backend -m netaudit.server` provides the smoke test.
